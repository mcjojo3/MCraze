...
public class LightingEngine implements Serializable {
	...
	private void init() {
		...
		if (isSun) {
			...
			for (int x = 0; x < width; x++) {
				for (int y = 0; y < height; y++) {
					// Null check to prevent NPE during world loading
					if (tiles[x][y] != null && tiles[x][y].type != null && tiles[x][y].type.lightEmitting > 0) {
						// Don't set lightValues here - let spreadLightingDijkstra do it
						// Otherwise the condition in spreadLightingDijkstra will skip the source
						lightFlow[x][y] = Direction.SOURCE;
						// Add to sources list so light propagates
						sources.add(new LightingPoint(x, y, Direction.SOURCE, tiles[x][y].type.lightEmitting));
						torchCount++;
					}

				}
			}
			System.out.println("LightingEngine.init: Found " + torchCount + " light sources, propagating...");
		}
	...
	public void addedTile(int x, int y) {
		...
		if (isSun) {
			...
			for (int i = 0; i < height; i++) {
				if (tiles[x][i] != null && tiles[x][i].type != null && tiles[x][i].type.lightBlocking != 0) {
					sun = false;
				}
				if (sun) {
		...
		} else if (tiles[x][y] != null && tiles[x][y].type != null && tiles[x][y].type.lightEmitting > 0) {
	...
	public List<LightingPoint> getSunSources(int column) {
		...
		for (int y = 0; y < height - 1; y++) {
			if (tiles[column][y] != null && tiles[column][y].type != null && tiles[column][y].type.lightBlocking != 0) {
				break;
			}
	...
	public class LightingPoint {
		...
		public List<LightingPoint> getNeighbors(boolean sun, int width, int height) {
			...
			int newValue = lightValue - 1 - lightBlocking;
